package com.zhy.selfdev.websocket;

import com.zhy.common.utils.SemaphoreUtils;
import com.zhy.common.utils.StringUtils;

import com.zhy.selfdev.websocket.callback.UavWebSocketCallback;
 ...
 */
@Component
@ServerEndpoint("/websocket/androiduav/{uid}")
public class UavWebSocketServer
{
    ...
    * 设置回调监听
    */
    private UavWebSocketCallback uavWebSocketCallback = new UavWebSocketCallback();
    ...
    public void onOpen(Session session,  @PathParam("uid") String uid) throws Exception
    {
        ...
        if (!semaphoreFlag)
        {
            ...
            UavWebSocketUsers.sendMessageToUser(session, String.format("当前在线人数超过限制数：%d", socketMaxOnlineCount));
            session.close();
        }
        else
        {
            ...
            uavWebSocketCallback.onOpenListener(uid);
        }
    }
    ...
    public void onClose(Session session, @PathParam("uid") String uid)
    {
        ...
        // 关闭时的监听
        uavWebSocketCallback.onCloseListener(uid);
    }
    ...
    public void onError(@PathParam("uid") String uid, Throwable exception) throws Exception
    {
        ...
        // 异常时的监听
        uavWebSocketCallback.onErrorListener(uid, exception);
    }
    ...
    public void onMessage(@PathParam("uid") String uid, String message)
    {
        ...
        // 接收到消息时的监听
        uavWebSocketCallback.onMessageListener(uid, message);
    }

    @OnMessage
    public void onMessage(@PathParam("uid") String uid, byte[] message) {
        LOGGER.info("\n 收到来自[{}]的字节流信息 - {}", uid, message);
        // 接收到消息时的监听
        uavWebSocketCallback.onMessageListener(uid, message);
    }
