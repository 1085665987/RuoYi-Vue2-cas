...
import com.zhy.selfdev.mavlink.common.v2.enums.GpsFixType;
...
public class Gps2Raw implements Message {
	...
	@MavlinkMessageParam(mavlinkType = "uint16_t", position = 6, typeSize = 2, streamLength = 2, description = "GPS HDOP horizontal dilution of position. If unknown, set to: UINT16_MAX", units = "cm")
	private int eph;

	@MavlinkMessageParam(mavlinkType = "uint16_t", position = 7, typeSize = 2, streamLength = 2, description = "GPS VDOP vertical dilution of position. If unknown, set to: UINT16_MAX", units = "cm")
	private int epv;
	...
	@MavlinkMessageParam(mavlinkType = "uint16_t", position = 9, typeSize = 2, streamLength = 2, description = "Course over ground (NOT heading, but direction of movement): 0.0..359.99 degrees. If unknown, set to: UINT16_MAX", units = "cdeg")
	private int cog;
	...
	@MavlinkMessageParam(mavlinkType = "uint32_t", position = 12, typeSize = 4, streamLength = 4, description = "Age of DGPS info", units = "ms")
	private long dgpsAge;

	@MavlinkMessageParam(mavlinkType = "uint16_t", position = 13, typeSize = 2, streamLength = 2, description = "Yaw in earth frame from north. Use 0 if this GPS does not provide yaw. Use 65535 if this GPS is configured to provide yaw and is currently unable to provide it. Use 36000 for north.", units = "cdeg")
	private int yaw;
	...
	public Gps2Raw(BigInteger timeUsec, short fixType, int lat, int lon, int alt, int eph, int epv, int vel, int cog, short satellitesVisible, short dgpsNumch, long dgpsAge, int yaw) {
	...
	public void messagePayload(byte[] messagePayload) {
		...
		yaw = byteArray.getUnsignedInt16(35);
	}
	...
	public byte[] messagePayload() {
		...
		byteArray.putUnsignedInt16(yaw,35);
	...
	public final Gps2Raw setYaw(int yaw) {
		this.yaw = yaw;
		return this;
	}
	...
	public boolean equals(Object o) {
		if (this == o) {
			...
			if (!Objects.deepEquals(this.timeUsec, other.timeUsec)) {
				...
				return false;
			} else if (!Objects.deepEquals(this.dgpsAge, other.dgpsAge)) {
				return false;
			} else {
