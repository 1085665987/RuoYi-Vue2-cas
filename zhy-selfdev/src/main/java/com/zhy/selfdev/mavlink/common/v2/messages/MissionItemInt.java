...
import com.zhy.selfdev.mavlink.common.v2.enums.MavCmd;
import com.zhy.selfdev.mavlink.common.v2.enums.MavMissionType;
...
public class MissionItemInt implements Message {
	...
	@MavlinkMessageParam(mavlinkType = "uint16_t", position = 3, typeSize = 2, streamLength = 2, description = "Waypoint ID (sequence number). Starts at zero. Increases monotonically for each waypoint, no gaps in the sequence (0,1,2,3,4).")
	private int seq;
	...
	@MavlinkMessageParam(mavlinkType = "int32_t", position = 12, typeSize = 4, streamLength = 4, description = "PARAM5 / local: x position in meters * 1e4, global: latitude in degrees * 10^7")
	private int x;

	@MavlinkMessageParam(mavlinkType = "int32_t", position = 13, typeSize = 4, streamLength = 4, description = "PARAM6 / y position: local: x position in meters * 1e4, global: longitude in degrees *10^7")
	private int y;

	@MavlinkMessageParam(mavlinkType = "float", position = 14, typeSize = 4, streamLength = 4, description = "PARAM7 / z position: global: altitude in meters (relative or absolute, depending on frame.")
	private float z;
	...
	public MissionItemInt(short targetSystem, short targetComponent, int seq, short frame, int command, short current, short autocontinue, float param1, float param2, float param3, float param4, int x, int y, float z, short missionType) {
	...
	public void messagePayload(byte[] messagePayload) {
		...
		x = byteArray.getInt32(25);
		y = byteArray.getInt32(29);
	...
	public byte[] messagePayload() {
		...
		byteArray.putInt32(x,25);
		byteArray.putInt32(y,29);
	...
	public final MissionItemInt setMissionType(short missionType) {
		this.missionType = missionType;
		return this;
	}
